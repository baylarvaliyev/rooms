...
export default function AppLayout({ children }: { children: React.ReactNode }) {
  ...
  return (
    <>
      <style>{`
        .app-shell { display: flex; height: 100vh; overflow: hidden; background: var(--bg0); }
        .sidebar { width: 70px; background: var(--bg1); border-right: 1px solid var(--border); display: flex; flex-direction: column; align-items: center; padding: 12px 0 14px; gap: 2px; flex-shrink: 0; z-index: 50; }
        .main-area { flex: 1; display: flex; flex-direction: column; overflow: hidden; min-width: 0; }
        .topbar { height: 52px; flex-shrink: 0; background: rgba(11,13,18,.95); backdrop-filter: blur(24px); border-bottom: 1px solid var(--border); display: flex; align-items: center; padding: 0 16px; gap: 10px; z-index: 40; }
        .page-wrap { flex: 1; overflow: hidden; display: flex; }
        ...
        @media (max-width: 768px) {
          ...
          .mobile-nav {
            ...
            height: 56px; background: rgba(11,13,18,.97);
          ...
          .page-wrap > * { padding-bottom: 70px !important; }
        }
      ...
      <div className="app-shell">
        ...
        <div className="main-area">
          ...
          <div className="topbar">
            ...
            <div style={{ display: 'flex', gap: '6px', alignItems: 'center' }}>
              {notifCount > 0 && (
                <div onClick={() => router.push('/notifications')} style={{ position: 'relative', cursor: 'pointer', fontSize: '20px', minWidth: '44px', minHeight: '44px', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                  🔔
              ...
              {profile?.username && (
                <div style={{ fontSize: '12px', color: 'var(--text3)', display: 'flex', alignItems: 'center', gap: '5px' }}>
                  {profile?.avatar_url && <div style={{ width: '22px', height: '22px', borderRadius: '50%', overflow: 'hidden' }}><img src={profile.avatar_url} style={{ width: '100%', height: '100%', objectFit: 'cover' }} /></div>}
                  <span style={{ display: 'none' }} id="topbar-username">@{profile.username}</span>
                </div>
              )}
            </div>
          </div>
        ...
        <div className="mobile-nav">
          {MOBILE_NAV.map(n => (
            <div key={n.id} onClick={() => router.push(n.path)} style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '3px', cursor: 'pointer', padding: '6px 12px', color: activeId === n.id ? 'var(--accent2)' : 'var(--text3)', position: 'relative', minWidth: '56px', flex: 1 }}>
